# Warehouse on L3s: the missing cell of the 2x2 (architecture x layout)
...
CLI = os.getcwd()
L3DIR = os.path.join(CLI, "data/trips/layout_compact/L3s")
FILES = sorted(glob.glob(f"{L3DIR}/**/*.parquet", recursive=True))
T0, T1 = "2026-01-15 08:00:00", "2026-01-16 08:00:00"
TMID = "2026-01-15 20:00:00"
BELT = "640730.0, 6042487.0, 654100.0, 6058230.0"
BLOCK = 8192
N_ITERS, TRIM, WARMUP = 5, 1, 1      # thesis harness convention
TABLE = "l3s_wh"
IDX = "l3s_wh_gist"
...
CLIPPED = f"""WITH clipped AS (
   SELECT mmsi, ship_type, atStbox(traj, {BELT_SB}) g FROM {TABLE}
   WHERE traj && {BELT_PB}),
...
QUERIES = {
    ...
    "harbour_entry": f"""SELECT count(DISTINCT mmsi) FROM (
        SELECT mmsi, atTime(traj, tstzspan('[{T0}, {T1})')) trip FROM {TABLE}
    ...
    "both_ports": f"""WITH rodby AS (SELECT DISTINCT mmsi FROM {TABLE}
         ...
         putt AS (SELECT DISTINCT mmsi FROM {TABLE}
...
KEYS = ("Shared Hit Blocks", "Shared Read Blocks", "Temp Read Blocks")

def trimmed(xs):
    xs = sorted(xs)
    xs = xs[TRIM:len(xs) - TRIM] if len(xs) > 2 * TRIM else xs
...
def build(cur):
    print(f"{len(FILES)} L3s files", flush=True)
    cur.execute(f"DROP TABLE IF EXISTS l3s_raw, {TABLE};")
    # file-only schema (11 cols); region_x/region_y/year/month are hive path columns and
    # are NOT in the file, so pg_parquet's positional mapping lines up exactly as for L0.
    cur.execute("""CREATE TABLE l3s_raw(mmsi bigint, ship_type text, segment_type text,
       traj bytea, tmin timestamptz, tmax timestamptz,
       bxmin float8, bxmax float8, ymin float8, ymax float8, dt date);""")
    t = time.time()
    for i, f in enumerate(FILES, 1):
        cur.execute(f"COPY l3s_raw FROM '{f}' (FORMAT parquet);")
        if i % 50 == 0:
            print(f"  copied {i}/{len(FILES)}", flush=True)
    load_s = time.time() - t
    t = time.time()
    cur.execute(f"""CREATE TABLE {TABLE} AS
       SELECT mmsi, ship_type, segment_type, tgeompointFromEWKB(traj) AS traj FROM l3s_raw;""")
    decode_s = time.time() - t
    cur.execute("DROP TABLE l3s_raw;")
    cur.execute(f"SELECT count(*) FROM {TABLE};")
    n = cur.fetchone()[0]
    t = time.time()
    cur.execute(f"CREATE INDEX {IDX} ON {TABLE} USING gist(traj);")
    cur.execute(f"ANALYZE {TABLE};")
    index_s = time.time() - t
    cur.execute(f"""SELECT pg_size_pretty(pg_relation_size('{TABLE}')),
                    pg_size_pretty(pg_relation_size('{IDX}')),
                    pg_total_relation_size('{TABLE}');""")
    heap, idx, total = cur.fetchone()
    print(f"loaded {n:,} tiled segments  copy {load_s:.1f}s + decode {decode_s:.1f}s "
          f"+ gist {index_s:.1f}s\n  heap={heap} index={idx} total={total/1e6:.0f} MB", flush=True)
    return {"rows": n, "load_s": round(load_s, 1), "decode_s": round(decode_s, 1),
            "index_s": round(index_s, 1), "total_bytes": total}
...
def main():
    ...
    for q in ORDER:
        ...
        rows.append({"backend": "mobilitydb_warehouse_L3s", "query": q, "answer": ans,
                     "runtime_ms": round(1000 * trimmed(ts), 1),
                     "blocks_total": blocks, "bytes_read": blocks * BLOCK,
                     "n_iters": N_ITERS})
        print(f"  {q:24} ans={str(ans):>9}  {rows[-1]['runtime_ms']:8.1f} ms  "
              f"{blocks*BLOCK/1e6:8.1f} MB", flush=True)

    df = pd.DataFrame(rows)
    df.to_csv("results/warehouse_l3s.csv", index=False)
    pd.DataFrame([meta]).to_csv("results/warehouse_l3s_build.csv", index=False)
    print(f"\nwrote results/warehouse_l3s.csv  (total {df.bytes_read.sum()/1e6:.0f} MB)")
