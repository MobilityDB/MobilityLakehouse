...
WITH porta AS (
  ...
  WHERE trip_xmin <= :axmax AND trip_xmax >= :axmin
    ...
    AND tgeo_at_stbox(hex(trip),
      'SRID=:srid;STBOX XT(((:axmin,:aymin),(:axmax,:aymax)),[:t0s,:t1s])', true) IS NOT NULL),
portb AS (
  SELECT DISTINCT mmsi FROM trips
  WHERE trip_xmin <= :bxmax AND trip_xmax >= :bxmin
    ...
    AND trip_tmin <= :t1 AND trip_tmax >= :t0 AND dt BETWEEN :d0 AND :d1
    AND tgeo_at_stbox(hex(trip),
      'SRID=:srid;STBOX XT(((:bxmin,:bymin),(:bxmax,:bymax)),[:t0s,:t1s])', true) IS NOT NULL)
