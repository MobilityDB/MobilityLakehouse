#!/usr/bin/env bash
# reproduce.sh — the planar benchmark of the lakehouse paper, from the DMA archives to the layouts.
#
...
# (planar/92_check_layouts.sh). The archives are the Danish Maritime Authority's and are fetched
# onto this machine first, with ingest/fetch_dma.sh; this script stops and names that command when
# a day of the period is neither downloaded nor already in the raw zone.
#
...
# 2026-01-15 2026-01-21 is one week of it. The run is data/stage/planar/<FROM>_<TO + 1 day>.
#
...
# built from them; STEPS ("ingest clean layouts check"), the steps this invocation runs, each
# reading what the step before it wrote.
set -euo pipefail
...
STEPS=${STEPS:-"ingest clean layouts check"}
