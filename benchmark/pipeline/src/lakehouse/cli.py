...
from lakehouse.ingest.dma import (
    ingest_dma_day,
    ingest_dma_range,
    ingest_dma_source,
)
from lakehouse.process.compact import compact_daily_to_monthly
from lakehouse.process.layouts import l0_file
from lakehouse.runner import (
    DuckdbRuntime,
    LAYOUTS,
    LayoutParams,
    build_l0_day,
...
def ingest_day(
    ...
    keep_zip: Annotated[bool, Option()] = False,
    keep_csv: Annotated[bool, Option()] = False,
    force: Annotated[bool, Option(help="Refetch even if data/raw already has it")] = False,
) -> None:
    settings = Settings.create()
    out = ingest_dma_day(
        parse_day(day), settings.pipeline.raw_dir,
        keep_zip=keep_zip, keep_csv=keep_csv, force=force,
    )
...
def ingest_range(
    ...
    keep_zip: Annotated[bool, Option()] = False,
    keep_csv: Annotated[bool, Option()] = False,
    force: Annotated[bool, Option(help="Refetch days already in data/raw")] = False,
) -> None:
    settings = Settings.create()
...
def build_l0(
    ...
    force: Annotated[bool, Option(help="Rebuild days already present in L0")] = False,
) -> None:
    settings = Settings.create()
    ...
    for d in days:
        done = l0_file(l0_root, d)
        if done.exists() and not force:
            print(f"[green]skip {d}: already in {done.parent}[/green]")
            continue
...
app = Typer(help="AIS mobility lakehouse: ingest, process, store, query.")
app.add_typer(
    cli,
    name="pipeline",
