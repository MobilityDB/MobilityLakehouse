# Compaction: concatenate a daily-granularity L1..L4 layout into one
...
from lakehouse._duckdb import open_pipeline_conn
from lakehouse.process.layouts import _concat_parquet
from lakehouse.runner import DuckdbRuntime
...
def compact_daily_to_monthly(
    *,
    ...
    print(
        f"  {layout}: {len(day_dirs)} day(s) x {len(partitions)} partition(s) "
        f"-> {dst_root}"
    )
