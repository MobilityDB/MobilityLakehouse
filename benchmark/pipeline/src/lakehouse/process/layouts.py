...
from lakehouse.store.geoparquet import (
    GEOPARQUET_EXCLUDE,
    GEOPARQUET_PROJECTION,
    kv_metadata_clause,
)
from lakehouse._params import PipelineParams
from lakehouse._utils import _sql_str
from lakehouse.process.tile import (
...
def write_l2_spacesplit(
    ...
    *,
    ...
    print(
        f"  L2: {len(cells)} candidate region cells x {len(l0_days)} day(s) "
...
def write_l3_mest(
    ...
    *,
    ...
    cells = enumerate_region_cells(con, region_size_m, l0_days, params.min_motion_points)
    print(
        f"  L3: {len(cells)} candidate region cells x {len(l0_days)} day(s) "
        f"(granularity={granularity})"
    )
...
def write_l4_timesplit(
    ...
    *, root: Path, year: int, month: int,
    ...
    print(f"  L4: {num_bins} bins x {bin_hours}h (granularity={granularity})")
