...
from lakehouse.query.registry import METRIC_EPSG, TRIPS_DEST
...
VARIANT = {"L1": "L1s", "L2": "L2s", "L3": "L3s", "L4": "L4s"}
    ...
    for src_name in (s.strip() for s in args.layouts.split(",")):
        ...
        for src in files:
            ...
            con.execute(f"""
                ...
                (FORMAT PARQUET, COMPRESSION ZSTD, ROW_GROUP_SIZE {rg}, {TEMPORAL_KV})
            """)
        nrg = con.execute(f"""
