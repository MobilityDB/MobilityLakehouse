...
from lakehouse._utils import _sql_str
...
def _extract_csv_from_zip(zip_path: Path, raw_dir: Path) -> Path:
    with ZipFile(zip_path, "r") as zf:
        ...
        with zf.open(csv_names[0]) as src, out_path.open("wb") as dst:
            copyfileobj(src, dst, 8 * 1024 * 1024)
    return out_path
...
def ingest_dma_day(
    ...
    *,
    ...
    existing = raw_dir / f"aisdk-{day.isoformat()}.parquet"
    if existing.exists() and not force:
        print(f"[ingest] {day.isoformat()}: already in {raw_dir}; skipped")
        return existing

    t0 = perf_counter()
    supplied_zip = zip_path.exists() and zip_path.stat().st_size > 0
    if supplied_zip:
        print(f"[ingest] {day.isoformat()}: using {zip_name} already in {raw_dir}")
    else:
    ...
    if not keep_zip and not supplied_zip:
        zip_path.unlink(missing_ok=True)
...
def ingest_dma_range(
    ...
    *,
    ...
    while current <= end_day:
        try:
            outputs.append(
                ingest_dma_day(current, raw_dir, keep_zip=keep_zip,
                               keep_csv=keep_csv, force=force)
            )
        except requests.HTTPError as e:
            if e.response is not None and e.response.status_code == 404:
                print(f"[ingest] {current.isoformat()}: not published by DMA; skipped")
                missing.append(current)
            else:
                raise
        current += timedelta(days=1)
    if missing:
        print(f"[ingest] {len(missing)} day(s) unavailable: "
              f"{missing[0].isoformat()} .. {missing[-1].isoformat()}")
    return outputs
